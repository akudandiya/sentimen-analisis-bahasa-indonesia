...
class DBM(object):
    ...
    def __init__(self,dataset,labels=np.array([]),
                batch_size = 50,
                layers=[6,12],
                fantasy_count = 100,
                learning_rate = .001, ):
        ...
        self.layers.append({
                            'size':self.features,
                            'fantasy': np.random.randint(0,2,(fantasy_count,self.features)).astype(float),
                            'mu':0,
                            'bias':self.sigma_inverse(np.mean(dataset, axis=0)).reshape(1,self.features),
                            })
    ...
    def l2_pressure(self,weights):
        norms = np.clip(np.sqrt(np.sum(weights*weights, axis=0)),.00001,10000)
        norms = np.floor(1/norms.reshape(norms.shape[0],1))
    ...
    def data_sample(self, num):
        if self.labels.shape[0] > 0:
            return (self.dataset[np.random.randint(0, self.dataset.shape[0], num)],
                self.labels[np.random.randint(0, self.labels.shape[0], num)])
        else:
            return (self.dataset[np.random.randint(0, self.dataset.shape[0], num)],
                np.array([]))
    ...
    def sample(self, fn, args):
        temp = fn(*args)
        temp_cutoff = np.random.rand(*temp.shape)
    ...
    def _predict(self,W,bias,inputs):
        return self.sigma(bias + np.dot(inputs,W))


    #The energy of a given layer with a given input and output vector
    def _energy(self,v,W,h,bv,bh):
        return np.mean(-np.dot(v,bv.T) -np.dot(h,bh.T))- np.tensordot(np.dot(v,W),h, axes=([0,1],[0,1]))
    ...
    def energy(self, v):
        hs =  [np.round(self.sigma(self.layers[1]['bias']+np.dot(v,self.layers[1]['W'])))]
        for i in range(2,len(self.layers)):
            hs.append(np.round(self.sigma(self.layers[i]['bias']+np.dot(hs[-1], self.layers[i]['W']))))
    ...
    def total_entropy(self):
        pred = np.clip(self.predict_probs(self.dataset),0.0001,.9999)
        return np.sum(self.labels*np.log(pred) + (1-self.labels)*np.log(1-pred))
    ...
    def prob_given_vis(self, W, vs,bias, double=False):
        if double:
            return self.sigma(2*(bias + np.dot(vs, W)))
        else:
            return self.sigma(bias + np.dot(vs, W))
    ...
    def prob_given_out(self, W, hs,bias,double=False):
        if double:
            return self.sigma(2*(bias + np.dot(hs, W.T)))
        else:
            return self.sigma(bias + np.dot( hs, W.T))
    ...
    def train_backprop(self, train_iterations=300, weight=1, layers = None):
    ...
    def backprop_step(self,data,labels,rate, 
        ...
        for layer in range(layers-1,min,-1):
            ...
            weight_errors =  np.mean((act-labels)*act*(1-act),axis=0)* use_W
            ...
            for iter in range(layers-1, layer,-1):
                ...
                prior_act = np.mean(prior_act*(1-prior_act), axis=0)
                prior_act = prior_act.reshape(prior_act.shape[0],1)
                weight_errors = prior_act*weight_errors
                weight_errors = np.dot(use_W, weight_errors)
            #output layer       
    ...
    def train_unsupervised(self, layer, train_iterations=300, gibbs_iterations=10):
        layers=len(self.layers)
        ...
        for iter in range(train_iterations):
            ...
            previous = np.round(self.predict_probs(data, omit_layers=layers-layer))
            bias = self.layers[layer]['bias']
            mu = bias+np.dot(previous,self.layers[layer]['W'])
            ...
            gradient_part = - 1.0/(self.batch_size) * np.dot(previous.T, mu)
            approx_part =- 1.0/self.fantasy_count * np.dot(self.layers[layer-1]['fantasy'].T,
    ...
    def gibbs_predict(self, input, mask=None,samples = 100,  gibbs_iterations=100, stop_layer = None):
        ...
        for i in range(1,stop_layer+2):
            input_state[i] = np.zeros((input.shape[0],self.layers[i]['W'].shape[1]))
        out = []
    ...
    def deterministic_predict(self, input, mask=None,  stop_layer = None):
        ...
        for i in range(1,stop_layer-1):
            ...
            bias = self.layers[i]['bias']
            W = self.layers[i]['W']
            input_state[i] = np.round(self.prob_given_vis(W,active,bias))
        for i in range(stop_layer-2,0,-1):
            active = input_state[i+1]
            bias = self.layers[i]['bias']
            W = self.layers[i+1]['W']
            input_state[i] = np.round(self.prob_given_out(W,active,bias))
    ...
    def add_layer(self, size): 
        ...
        hidden['W'] = np.random.randn(above,size)
        hidden['bias'] = np.random.randn(1,size)
        hidden['momentum'] = np.zeros((above,size))
        hidden['fantasy'] = np.random.randint(0,2,(self.fantasy_count,size)).astype(float)
